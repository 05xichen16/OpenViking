...
Use KMM as OpenClaw's long-term context engine: automatic recall, session archive, memory extraction, semantic search, and RAG over a remote KMM server.

## Quick Start

```bash
openclaw plugins install clawhub:@kmm/openclaw-plugin
openclaw kmm setup --base-url http://my-server:1933 --api-key sk-xxx --json
...
> Install the OpenClaw plugin @kmm/openclaw-plugin for KMM remote memory. My server is at `http://my-server:1933` and my API key is `sk-xxx`.
...
| Stage | What happens |
|-------|-------------|
| **Every turn** (`afterTurn`) | New messages are appended to a KMM session; commit/extraction is threshold-triggered |
...
| `kmm_archive_search` | Search across archives by keyword |
| `kmm_archive_expand` | Expand an archive back to raw messages |
| `kmm_recall_trace` | Inspect why recall/search returned or injected specific results |
| `add_resource` | Import documents, URLs, or Git repos when explicitly enabled |
| `add_skill` | Import KMM skills |
| `kmm_search` | Search imported resources and skills |
| `kmm_read` | Read the full original content of one exact KMM URI |
| `kmm_multi_read` | Read the full original content of multiple KMM URIs |
| `kmm_list` | List KMM directories after search to inspect sibling chunks and overview files |
| `kmm_tool_result_read` | Restore the full original content of an externalized tool result |
| `kmm_tool_result_search` | Search inside an externalized tool result by keyword |
| `kmm_tool_result_list` | List externalized tool results in the current session |
...
- **Where it goes**: Your configured KMM server (`baseUrl`). The plugin only sends data to that server; downstream model/provider data handling (embedding, VLM) depends on the server's configuration.
- **Storage**: All data lives on your KMM server under `viking://user/*` (including `viking://user/sessions/*`) and `viking://resources/*`.
- **API Key**: Sent as `X-API-Key` header over your configured connection. Never logged or forwarded.
- **Multi-tenant isolation**: Supports `accountId` and `userId`. Optional `peer_role` / `peer_prefix` controls whether OpenClaw speakers are written as KMM `peer_id`.

## Verify

```bash
openclaw kmm status --json     # one-shot health check
openclaw config get plugins.slots.contextEngine  # should output: kmm
```
...
| [docs/kmm-tos-install-guide.md](./docs/kmm-tos-install-guide.md) | TOS release bundle publishing and installer guide |
| [docs/kmm-openclaw-plugin-guide.md](./docs/kmm-openclaw-plugin-guide.md) | Comprehensive Chinese guide for usage, configuration, debugging, testing, build, release, deployment, and rollback |
| [docs/kmm-websocket-rpc-api.md](./docs/kmm-websocket-rpc-api.md) | Gateway WebSocket RPC usage for KMM tools |
| [docs/kmm-runtime-query-config.md](./docs/kmm-runtime-query-config.md) | Runtime query config scopes, fields, and commands |
| [docs/kmm-install-package-contract.md](./docs/kmm-install-package-contract.md) | Package and install contract verification notes |

> **Plugin vs Skill**: This page is for `@kmm/openclaw-plugin` (the context-engine plugin). Do **not** use `clawhub install kmm` — that installs a different AgentSkill.
...
This plugin is registered as the `kmm` context engine in OpenClaw.

## Design Positioning

- OpenClaw still owns the agent runtime, prompt orchestration, and tool execution.
- KMM owns long-term memory retrieval, session archiving, archive summaries, and memory extraction.
...
- tool provider: registers memory/archive tools plus KMM resource and skill import tools
- runtime manager: connects to and monitors a remote KMM service

## Overall Architecture

![Overall OpenClaw and KMM plugin architecture](./images/openclaw-plugin-engine-overview.png)
...
- All HTTP traffic goes through `KMMClient`, which centralizes tenant headers and routing logs.
- The KMM service owns sessions, memories, archives, and Phase 2 extraction, with storage under `viking://user/*` (including `viking://user/sessions/*`) and `viking://resources/*`.

That split lets OpenClaw stay focused on reasoning and orchestration while KMM becomes the source of truth for long-lived context.

## Identity and Routing

The plugin keeps OpenClaw session identity in session and peer metadata. It does not send a KMM agent identity or create an agent namespace.
...
- data-plane recall/search/read/import/delete sends the same resolved peer identity as a tenant peer-identity header when `peer_role` is `assistant` or `person`
- when OpenClaw does not provide a session agent, use its default agent `main` for local session and assistant peer metadata
- only add the tenant account/user identity headers when `accountId` / `userId` are explicitly configured

This matters because KMM tenant identity is account/user-scoped, while OpenClaw agent identity is runtime metadata.
...
The plugin writes and searches user-scoped memory through `viking://user/...`; KMM resolves that alias from the request tenant and actor-peer context. Deprecated agent URI paths are not used by the plugin.
...
1. Preflight assemble: params include `prompt`; `messages` is still old history. The plugin reads archive/session context back from KMM and rebuilds history.
...
3. Runs a quick availability precheck so model requests do not stall when KMM is unavailable.
4. Queries the configured `recallTargetTypes` (`user,agent` by default; optionally `resource`; use `kmm_archive_search` and `kmm_archive_expand` for session history).
5. Deduplicates, threshold-filters, reranks, and trims the results under a token budget.
6. Prepends the selected memories as a `## Long-term Memories` section inside `<kmm-context>` to the current user message; it does not append a standalone synthetic user message.
...
During preflight, `assemble()` is not just replaying old chat history. It reads session context back from KMM under a token budget, then rebuilds OpenClaw-facing messages:
...
`afterTurn()` has a narrower job: append only the new turn into the KMM session.
...
- it strips injected `<kmm-context>` blocks, historical `<relevant-memories>` blocks, and metadata noise before capture
- it appends the sanitized turn text into the KMM session
...
- if `memory_store` commits but extracts 0 memories, check the KMM server extraction/model configuration; the explicit path triggered extraction, but the extractor did not produce a memory
...
- if the summary is too coarse, the model can call `kmm_archive_expand` to reopen a specific archive
...
- `memory_recall`: explicit long-term memory search
- `memory_store`: write explicit long-term facts into a KMM session and trigger commit
- `memory_forget`: delete by URI, or search first and remove a single strong match
- `kmm_archive_expand`: expand a concrete archive back into raw messages
- `kmm_recall_trace`: inspect recent recall/search trace records when `traceRecall` is enabled
- `add_resource`: import a document, directory, URL, or Git repository as a KMM resource when explicitly enabled
- `add_skill`: import or register a KMM skill
- `kmm_search`: search KMM resources and skills, especially after importing them
- `kmm_read`: read one exact `viking://` URI returned by `kmm_search` or `kmm_list`
- `kmm_multi_read`: read multiple exact `viking://` URIs, useful for an overview plus sibling chunks
- `kmm_list`: list a hit's parent directory after `kmm_search` to recover sibling chunks, `.overview.md`, and related split-document context
...
- `kmm_archive_expand` is the "go back to archive detail" escape hatch when summaries are not enough
...
- `add_skill` imports skills into KMM, while `add_resource` imports resources
- `kmm_search` closes the loop after import by letting the user or agent confirm and consume resources and skills
- `kmm_read` turns a ranked hit into original evidence before answering precise documentation, codebase, configuration, or procedural questions
- `kmm_multi_read` reads overview and sibling chunks together when a split document needs more context than a single hit
- `kmm_list` complements `kmm_search` when a ranked hit is only one chunk of a larger procedure or document

`kmm_archive_expand` is especially important because `assemble()` normally returns archive summaries and indexes, not the full raw transcript.

### Resource and Skill Import

Resource and skill imports are intentionally separate because they land in different KMM namespaces and use different server APIs:
...
/add-skill ./skills/install-kmm-memory --wait
/kmm-search "KMM install" --uri viking://resources/kmm-readme
/kmm-search "memory install skill" --uri viking://user/skills
```
...
`restore` (alias `resume`) *hydrates* the chosen KMM session into OpenClaw's **local**
...
KMM's `sessionId` equals OpenClaw's session UUID, the plugin fetches the session's
...
entry (the store is backed up to `sessions.json.kmm.bak` first). This is what makes
...
leading summary message; assistant provider/usage accounting is defaulted (KMM does not
retain the original values).

Resource import supports remote URLs, Git URLs, local files, local directories, and uploaded zip files. KMM's built-in parsers cover common documents and media such as Markdown, text, PDF, HTML, Word, PowerPoint, Excel, EPUB, images, audio, and video. Directory imports also accept common code, documentation, and config file extensions such as `.py`, `.js`, `.ts`, `.go`, `.rs`, `.java`, `.cpp`, `.json`, `.yaml`, `.toml`, `.csv`, `.rst`, `.proto`, `.tf`, and `.vue`.

For HTTP safety, the plugin never sends a direct local filesystem path to the KMM server. Local files and directories are first uploaded through `/api/v1/resources/temp_upload`; directories are zipped locally with a pure JavaScript zip implementation before upload.
...
The KMM service must be deployed and running independently before the plugin can connect to it.
...
- in the current version, `afterTurn()` already appends to the KMM session, but commit remains threshold-triggered and asynchronous on that path
...
```bash
cat ~/.kmm/data/log/kmm.log
```

### Web Console

```bash
python -m kmm.console.bootstrap --host 0.0.0.0 --port 8020 --kmm-url http://127.0.0.1:1933
```
...
| --- | --- | --- |
| `plugins.slots.contextEngine` is not `kmm` | The plugin slot was never set, or another plugin replaced it | `openclaw config get plugins.slots.contextEngine` |
| Cannot connect to KMM service | `baseUrl` is wrong or the service is down | Check `baseUrl` in config and test connectivity manually |
...
| long chats stop extracting memory | `pending_tokens` never crosses the threshold, or Phase 2 fails server-side | Check plugin config and `~/.kmm/data/log/kmm.log` |
| summaries are too coarse for detailed questions | You need archive-level detail, not just summary | Use an ID from `[Archive Index]` with `kmm_archive_expand` |
